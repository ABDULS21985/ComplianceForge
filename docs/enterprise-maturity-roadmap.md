...
2. **Partial** — Maintain a versioned product capability catalogue with owner, maturity, dependencies, and availability by subscription tier.
3. **Queued** — Publish and validate an OpenAPI 3.1 contract for every browser, automation, and public-portal endpoint.
...
7. **Queued** — Add tenant-aware feature flags with percentage rollout, prerequisites, kill switches, and an audited admin UI.
8. **Partial** — Enforce subscription entitlements and usage limits server-side, with graceful upgrade UX instead of hidden navigation alone.
9. **Queued** — Add tenant-level data retention, archival, residency, and legal-hold policy configuration.
10. **Queued** — Add an administrator diagnostics centre showing dependencies, worker lag, migrations, connectors, and safe configuration checks.
...
18. **Queued** — Implement SCIM 2.0 users and groups with bearer-token rotation, deprovisioning, pagination, and conformance tests.
19. **Queued** — Add TOTP and WebAuthn/passkey MFA, recovery codes, step-up authentication, and tenant enforcement policies.
20. **Queued** — Add secure invitations, email verification, forgotten-password recovery, session/device inventory, and global sign-out.
...
25. **Queued** — Add full custom-role CRUD, permission matrix UX, role cloning, validation, and impact previews.
26. **Partial** — Complete the ABAC decision point and policy administration path using typed contracts and request-scoped database access.
27. **Queued** — Enforce object-level grants for assigned owners, departments, locations, and time-limited external collaborators.
28. **Queued** — Enforce field-level visibility and masking for personal, financial, legal, and security-sensitive attributes.
29. **Queued** — Add time-boxed external-auditor access with sponsor approval, automatic expiry, download restrictions, and watermarking.
30. **Queued** — Add access reviews, segregation-of-duties rules, privileged-access workflows, and immutable certification evidence.
...
37. **Partial** — Add reliable focus management, announcements, accessible dialogs, touch targets, reduced motion, and non-colour status cues.
...
45. **Partial** — Complete malware-scanned object uploads, checksums, content validation, signed downloads, quarantine, and storage quotas.
46. **Queued** — Add evidence versioning, supersession, expiry, reviewer sign-off, chain of custody, legal hold, and immutable history.
47. **Partial** — Run scheduled evidence collectors with connector credentials, validation criteria, retries, freshness, and provenance.
...
61. **Partial** — Promote incident CRUD, timeline, assignments, categorisation, severity, containment, and closure to required production composition.
...
69. **Queued** — Add retention schedules with defensible deletion, holds, exceptions, evidence, and processor propagation.
...
71. **Partial** — Promote vendor CRUD, ownership, criticality, service details, data processing, DPA, certification, and lifecycle states.
...
78. **Queued** — Add asset inventory with ownership, classification, lifecycle, relationships, criticality, and control coverage.
...
116. **Queued** — Add tamper-evident audit events with actor, tenant, request, before/after, reason, source, hash chaining, and export.
...
118. **Queued** — Add legal hold across evidence, policies, incidents, audits, comments, reports, and deletion jobs.
...
129. **Queued** — Add secure SDLC threat models, architecture risk records, security champions, review gates, and penetration-test cadence.
130. **Queued** — Add vulnerability intake, ownership, exploitability analysis, remediation SLAs, exceptions, and customer advisories.
...
142. **Queued** — Instrument API, database, Redis, queue, workers, schedulers, storage, and connectors with OpenTelemetry traces.
143. **Queued** — Expose Prometheus metrics for RED/USE signals, business jobs, deadlines, delivery health, and tenant-safe aggregates.
144. **Queued** — Define service-level indicators and objectives for availability, latency, correctness, freshness, and job completion.
145. **Queued** — Add multi-window burn-rate alerts tied to actionable runbooks, ownership, escalation, and maintenance windows.
146. **Queued** — Add dashboards for API saturation, slow queries, pool use, cache performance, queues, worker lag, and storage.
147. **Queued** — Add distributed correlation across browser BFF, API, outbox, queue, worker, connector, and outbound delivery.
148. **Queued** — Add tenant-safe support bundles with configuration fingerprints, health snapshots, redaction, and customer consent.
...
156. **Queued** — Add property/fuzz tests for parsers, filters, state machines, permissions, webhooks, uploads, and event envelopes.
157. **Queued** — Add concurrency tests for reference allocation, refresh rotation, idempotency, leases, approvals, and transitions.
...
182. **Queued** — Add user/group lifecycle administration, bulk import, manager hierarchy, suspension, reactivation, and ownership transfer.
...
189. **Queued** — Add records-management classification, retention disposition approvals, export controls, and litigation hold reporting.
